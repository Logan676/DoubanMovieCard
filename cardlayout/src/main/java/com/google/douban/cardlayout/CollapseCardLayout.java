 ...
 */
@SuppressLint("NewApi")
public class CollapseCardLayout extends ViewGroup {
    ...
    private CollapseCardAdapter mAdapter;
    ...
    public CollapseCardLayout(Context context) {
        this(context, null);
    }

    public CollapseCardLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CollapseCardLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        if (attrs != null) {
            int defStyleRes = 0;
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CollapseCardLayout,
                    defStyleAttr, defStyleRes);

            int contentML = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_childrenContentMarginLeft, CHILDREN_CONTENT_MARGIN_LEFT);
            setChildrenContentMarginLeft(contentML);

            int ML = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_itemMarginLeft, ITEM_MARGIN_LEFT);
            int MR = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_itemMarginRight, ITEM_MARGIN_RIGHT);
            int MT = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_itemMarginTop, ITEM_MARGIN_TOP);
            int MB = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_itemMarginBottom, ITEM_MARGIN_BOTTOM);
            setItemMarginLeft(ML);
            setItemMarginRight(MR);
            setItemMarginTop(MT);
            setItemMarginBottom(MB);

            int w = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_cardWidth, W);
            int h = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_cardHeight, H);

            setCardViewWidth(w);
            setCardViewHeight(h);

            float scale = a.getFloat(R.styleable.CollapseCardLayout_cardScale, SCALE_RATIO_L4);
            setSaleRatio(scale);

            float alpha = a.getFloat(R.styleable.CollapseCardLayout_cardAlpha, ALPHA_RATIO_L3);
            setAlpha(alpha);

            int dividerW = a.getDimensionPixelOffset(R.styleable.CollapseCardLayout_itemDividerWidth, ITEM_DIVIDER_W);
    ...
    public void setAdapter(CollapseCardAdapter adapter) {
        resetList();
        mRecycler.clear();
